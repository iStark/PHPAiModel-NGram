<?php
/*
 * PHPAiModel-NGram — generator_mode.php
 * N-gram weights generator (RU/EN) with extended intents ×10.
 *
...
// ---------- extended intents (×10) ----------
// IMPORTANT: each entry is an array of TWO arrays: [ [user variants], [assistant variants] ]
$ru_intents = [
    "greet" => [
...
// ---------- stop transitions after punctuation ----------
foreach (['.','!','?'] as $p) {
...
$weights = [
    ...
    "meta" => [
        ...
        "note"    => "generated by generator_mode.php; no token 'Assistant'; turns={$turns}; seed={$seed}",
        "created" => time(),
        "size_tokens" => $L,
        "turns"  => $turns,
    ],
...
if ($bytes === false) {
    ...
    echo json_encode([
        ...
        "php_error" => $err,
        "hint" => "Check write permissions/quota/open_basedir",
    ], JSON_UNESCAPED_UNICODE);
    exit;
}
